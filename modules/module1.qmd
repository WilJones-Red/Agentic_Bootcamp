...
You can now run the below code to install needed packages and load the environment variables from `.env` into this file. (You will need to rerun `load_dotenv()` every time you open the file again, but not `pip install`, which is a one time run.)
...
# Mac:
# %pip3 install -q -U langchain langchain-google-genai python-dotenv
```
